...
| Compilation blocks the render thread | none — open at release | Measured at the app's real settings (120 Hz, loop window): 5 ms for the default Composition, **164 ms** for the concurrent-Wheels reference, on every edit — roughly 6 fps while dragging. No benchmark will catch it: the work is not excessive, its thread is. `useDeferredValue` would not fix it either, since React cannot interrupt one 164 ms synchronous call. A Web Worker is the fix and is affordable — `structuredClone` of the whole compiled performance costs 1.7 ms, about 1%. Not yet done. | Open |
