...
| The `rest` flag on a performed event is written but never read | MG-21 | `NoteMusicalEvent.rest` is set by the compiler when variation silences a note, but no consumer honours it: the live scheduler, the MIDI exporter, the Strudel exporter, and now the offline renderer all sound it anyway. Found during MG-20. The renderer deliberately matches live playback rather than diverging on its own; the fix has to change live and offline together and spans files outside MG-20's list. | Open |
