...
describe('Transport-beat adapters', () => {
  it('quantizes absolute beats rather than normalized curve positions', () => {
    expect(
      quantizeAbsoluteBeat(9.3, { gridBeats: 0.5, strength: 1 }),
    ).toBe(9.5)
    expect(
      quantizeAbsoluteBeat(9.3, { gridBeats: 0.5, strength: 0.5 }),
    ).toBe(9.4)
    expect(
      quantizeAbsoluteBeat(9.3, { gridBeats: 0.5, strength: 0 }),
    ).toBe(9.3)
  })
