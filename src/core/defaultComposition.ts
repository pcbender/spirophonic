...
export const referenceComposition = {
  ...
  fields: [
    ...
    },
    {
      ...
      rotation: 0,
      boundaries: [
        { id: 'spoke-1', name: 'Spoke E', enabled: true, index: 0, kind: 'spoke' as const, angle: 0, length: 200 },
        { id: 'spoke-2', name: 'Spoke N', enabled: true, index: 1, kind: 'spoke' as const, angle: Math.PI / 2, length: 200 },
        { id: 'spoke-3', name: 'Spoke W', enabled: true, index: 2, kind: 'spoke' as const, angle: Math.PI, length: 200 },
        { id: 'spoke-4', name: 'Spoke S', enabled: true, index: 3, kind: 'spoke' as const, angle: (3 * Math.PI) / 2, length: 200 },
      ],
    },
  ],
  soundBanks: [{ ...bundledSoundBank }],
