 ...
 *
 * No *musical* bank ships with this repository. Sound banks are user-local by
 * decision — architectural invariant 11 keeps SF2/SF3 bytes out of the
 * Composition, and MG-10 settled that redistributing a General MIDI bank needs
 * a licence this project does not hold. So the showcase names a bank rather
 * than carrying one: with a matching bank imported, all four Instruments sound;
 * without one, MG-11's missing-bank isolation reports it and the three native
 * Instruments still play.
 *
 * For testing, see {@link sampleSoundBankBytes} — a real 890-byte SoundFont
 * generated by `spessasynth_core` under a licence we already hold.
 */
export const showcaseSoundBankDigest = '0'.repeat(64)
  ...
  composition.soundBanks = [
    {
      id: 'bank-showcase',
      name: 'Showcase Bank',
      digest: showcaseSoundBankDigest,
      format: 'sf2',
      source: 'local',
